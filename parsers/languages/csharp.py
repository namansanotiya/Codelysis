"""
C# Source Parser.
Extracts classes, interfaces, methods, and call sites.
"""
import re
...
class CSharpParser(BaseParser):
    def __init__(self):
        self.class_re = re.compile(
            r"^\s*(?:(?:public|private|protected|internal|abstract|sealed|static)\s+)*(?:class|interface|struct)\s+([A-Za-z0-9_]+)"
        )
        self.method_re = re.compile(
            r"^\s*(?:(?:public|private|protected|internal|static|virtual|override|async)\s+)*"
            r"(?:[\w<>\[\],\s\?]+)\s+([A-Za-z0-9_]+)\s*\([^;{]*\)\s*(?:where\s+[\w:\s]+)?\{"
        )
    ...
    def parse(self, file_path: str, content: str) -> Tuple[List[CodeNode], List[Tuple[CodeNode, Optional[str]]]]:
        ...
        for idx, line_str in enumerate(source_lines, start=1):
            ...
            # 1. Class / Interface / Struct
            class_match = self.class_re.search(clean)
            if class_match:
                cname = class_match.group(1)
                if not is_builtin_or_stdlib(cname, "csharp"):
            ...
            if method_match:
                ...
                if not is_builtin_or_stdlib(mname, "csharp"):
            ...
            for call_match in self.call_re.finditer(clean):
                ...
                if not is_builtin_or_stdlib(called, "csharp"):
